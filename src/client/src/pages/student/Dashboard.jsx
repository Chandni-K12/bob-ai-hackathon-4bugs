      ...
      <motion.div variants={item} className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="sm:ml-auto flex items-center gap-2">
          <div className="px-3 py-1.5 rounded-full bg-eco-amber/10 text-eco-amber text-sm font-medium flex items-center gap-1.5">
            <Star className="w-3.5 h-3.5" /> Level {dashboard?.level ?? user?.level ?? 0}
          </div>
        </div>
      </motion.div>

      <motion.div variants={container} className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        <MetricCard emoji="⚡" label="Eco Points" value={formatNumber(dashboard?.points ?? user?.points ?? 0)} sub="+180 this week" color="text-eco-green" gradient="bg-eco-green" />
        <MetricCard emoji="🔥" label="Weekly Streak" value={`${dashboard?.streak ?? user?.streak ?? 0} Weeks`} sub="Streak is active!" color="text-eco-orange" gradient="bg-eco-orange" />
        <MetricCard emoji="🏆" label="Class Rank" value={`#${dashboard?.classRank ?? user?.classRank ?? 7}`} sub="↑ 1 position" color="text-eco-blue" gradient="bg-eco-blue" />
        <MetricCard emoji="🏫" label="School Rank" value={`#${dashboard?.schoolRank ?? user?.schoolRank ?? 24}`} sub="↑ 3 positions" color="text-eco-purple" gradient="bg-purple-500" />
      </motion.div>
