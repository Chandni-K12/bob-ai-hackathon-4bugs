  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6 max-w-4xl mx-auto">
          ...
          <div className="text-center sm:text-left">
            ...
            <div className="flex items-center gap-3 mt-2 justify-center sm:justify-start">
              <span className="px-3 py-1 rounded-full bg-eco-amber/10 text-eco-amber text-sm font-medium flex items-center gap-1">
                <Star className="w-3.5 h-3.5" /> Level {profile?.level ?? user?.level ?? 1}
              </span>
              <span className="px-3 py-1 rounded-full bg-eco-green/10 text-eco-green text-sm font-medium">
                {formatNumber(profile?.points ?? user?.points ?? 0)} Eco Points
              </span>
            </div>
          </div>
        </div>
      ...
      <motion.div variants={container} className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { icon: Trophy, label: 'Class Rank', value: (profile?.classRank || user?.classRank) ? `#${profile?.classRank || user?.classRank}` : '-', color: 'text-eco-blue' },
          { icon: School, label: 'School Rank', value: (profile?.schoolRank || user?.schoolRank) ? `#${profile?.schoolRank || user?.schoolRank}` : '-', color: 'text-eco-purple' },
          { icon: Flame, label: 'Streak', value: `${profile?.streak ?? user?.streak ?? 0} weeks`, color: 'text-eco-orange' },
          { icon: Award, label: 'Badges', value: profile?.badges ?? user?.badges ?? 0, color: 'text-eco-gold' },
          { icon: Target, label: 'Missions', value: profile?.completedMissions ?? user?.completedMissions ?? 0, color: 'text-eco-green' },
          { icon: BarChart3, label: 'Quiz Accuracy', value: `${profile?.quizAccuracy ?? user?.quizAccuracy ?? 0}%`, color: 'text-eco-teal' },
          { icon: BookOpen, label: 'Learning', value: `${profile?.learningProgress ?? user?.learningProgress ?? 0}%`, color: 'text-eco-blue' },
          { icon: Zap, label: 'Eco Points', value: formatNumber(profile?.points ?? user?.points ?? 0), color: 'text-eco-green' },
        ].map((stat, i) => (
