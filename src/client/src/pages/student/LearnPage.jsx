              ...
              {lessons.slice(0, selectedTopic.lessons).map((lesson, i) => {
                ...
                return (
                  <motion.div key={i} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }} className={`relative z-10 flex flex-col items-center ${marginClass}`}>
                    ...
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold border-4 shadow-md transition-all ${completed ? 'bg-eco-green border-emerald-600 text-white border-b-8 active:translate-y-1 active:border-b-4' : active ? 'bg-eco-blue border-sky-600 text-white border-b-8 ring-4 ring-sky-500/20 active:translate-y-1 active:border-b-4 animate-pulse' : 'bg-slate-800 border-slate-900 text-slate-500 cursor-not-allowed border-b-8'}`}>
